...
class CommunityLearner:
    ...
    def process_session_completion(session: ProblemChainSession):
        ...
        try:
            ...
            # Define success criteria - RELAXED for testing
            is_high_quality_session = (
                overall_score >= 2.5 and  # Lowered from 4.0 to 2.5
                session_rating >= 2 and   # Lowered from 4 to 2
                session.is_completed      # Actually finished
            )
            
            # DEBUG: Log criteria evaluation
            logger.info(f"Session {session.session_id} quality check: overall_score={overall_score}, session_rating={session_rating}, completed={session.is_completed}, meets_criteria={is_high_quality_session}")
    ...
    def get_learning_insights(difficulty: str) -> Dict:
        ...
        """
        try:
            # Get recent successful examples (convert to list to avoid slice issues)
            successful_examples_list = list(CommunityLearningData.get_successful_examples(difficulty))
            
            if not successful_examples_list:
                return {
                    'has_data': False,
            ...
            # Get best performing examples (filter from the list)
            best_examples = [
                example for example in successful_examples_list 
                if example.overall_score >= 4.5
            ][:3]
            
            insights = {
                'has_data': True,
                'total_examples': len(successful_examples_list),
                'patterns': patterns,
                'best_examples': [
                    {
                ...
                'success_categories': list(
                    set(example.problem_category for example in successful_examples_list if example.problem_category)
                ),
                'avg_engagement_time': sum(ex.engagement_time for ex in successful_examples_list if ex.engagement_time) / len([ex for ex in successful_examples_list if ex.engagement_time]) if any(ex.engagement_time for ex in successful_examples_list) else 0
            }
